...
class BasketSystem {
    ...
    init() {
        ...
        document.addEventListener('keydown',(e) => {
            if (e.key === 'Escape' && this.basketOverlay.classList.contains('active')) {
                this.closeBasket()
            }
        })

        this.basketItemsContainer.addEventListener('click', (e) => {this.deleteProductBasket(e)})
    ...
    positionBasket() {
        if (!this.basketButton || !this.basketContainer) return;
    ...
    `;
        ...
        const orderButton = document.querySelector('.basket__order-button');
        this.updateBasketButton();
    }
    ...
    renderProductsBasket(data) {
        ...
        const basketItemsHtml = data.map(product => {
    ...
    deleteProductBasket(event) {
        ...
        setTimeout(() => {
            ...
            document.dispatchEvent(new CustomEvent('basketItemRemoved', {
                detail: { removedId: id }
            }));
            this.updateBasketButton();
        },300)
