...
class Form {
  ...
  constructor() {
    ...
    this.stepsElement = this.selectors.steps.map((selector) =>
      this.root.querySelector(selector),
    )
    this.nextButtonElement = Array.from(
      this.root.querySelectorAll(this.selectors.nextButton),
    )
    this.submitButtonElement = this.root.querySelector(
      this.selectors.submitButton,
    )
    this.phoneContentElement = this.root.querySelector(
      this.selectors.phoneContent,
    )
    this.currentStep = 0
  ...
  checkCurrentStep() {
    ...
    switch (this.currentStep) {
      ...
      case 1:
        return (
          currentStepElement.querySelectorAll('input[type="checkbox"]:checked')
            .length > 0
        )
      case 2:
        return currentStepElement.querySelector("textarea").value.trim() !== ""
      case 3:
        const emailElement = currentStepElement
          .querySelector(this.selectors.mail)
          .value.trim()
