...
function showErrorMessageForm(message,step) {
    ...
    existErrors.forEach((error) => {error.remove()})
...
function checkBucketValue(productsData) {
    ...
    bucket.forEach((bucketID,index) => {
        const existsInProducts = productsData.some(item => item.id === Number(bucketID));
        if (!existsInProducts) {
...
export {showErrorMessage,getBucketLocalStorage,setBucketLocalStorage,checkBucketValue,showErrorMessageForm};